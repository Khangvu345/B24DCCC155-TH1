  ...
  return (
    <Form form={form} layout="vertical" onFinish={onFinish} style={{ padding: 24 }}>
      ...
      <Form.Item name="soVanBangId" label="Sổ văn bằng" rules={[{ required: true }]}>
        <Select>
          {(soVanBangModel.danhSach || []).map((item: any) => (
            <Select.Option key={item._id || item.id} value={item._id || item.id}>{item.tenSo}</Select.Option>
          ))}
        </Select>
      </Form.Item>
...
const QuyetDinhPage: React.FC = () => {
  ...
  const columns: IColumn<QuyetDinhTotNghiep>[] = [
    { title: 'Số hiệu QĐ', dataIndex: 'soSQ', width: 150, filterType: 'string' },
    {
      title: 'Ngày ban hành', dataIndex: 'ngayBanHanh', width: 150,
      render: (val: any) => val ? moment(val).format('DD/MM/YYYY') : '-',
    },
    { title: 'Trích yếu', dataIndex: 'trichYeu', width: 300 },
    {
      title: 'Sổ văn bằng', dataIndex: 'soVanBangId', width: 200,
      render: (val: any) => (soVanBangModel.danhSach || []).find((i: any) => (i._id || i.id) === val)?.tenSo || '-',
    },
