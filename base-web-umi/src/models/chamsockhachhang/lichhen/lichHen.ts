...
import {getLichHen, addLichHen, updateLichHen, deleteLichHen} from "@/services/ChamSocKhachHang/LichHen/lichHen";
...
export default () => {
    ...
    const getModel = async (customParams?: any) => {
        try {
            setLoading(true);
            const res = await getLichHen({page,limit, ...customParams});
            ...
            console.log(e);
            message.error('Lấy dữ liệu lịch hẹn thất bại');
        } finally {
            setLoading(false);
        }
    } 
    ...
    const postModel = async (value: any) => {
        try {
            setLoading(true);
            const res = await addLichHen(value);
            if (res?.data?.errorCode === 'BAD_REQUEST') {
                message.error(res?.data?.info || 'Lịch hẹn bị trùng!');
                return;
            }
            if (res?.data?.data?.result) {
    ...
    const putModel = async (id: string, value: any) => {
        try {
            setLoading(true);
            const res = await updateLichHen({_id: id, ...value});
            if (res?.data?.data?.result) {
    ...
    const deleteModel = async (id: string) => {
        try {
            setLoading(true);
            const res = await deleteLichHen({_id: id});
            if (res?.data?.data?.result) {
